...
CERT_FONT_PATH = os.path.join(_ASSETS_DIR, "NotoSerifKR.ttf")
CERT_TEMPLATE_PATH = os.path.join(_ASSETS_DIR, "cert_template.pdf")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        "정산양식 엑셀 파일을 보내주시면, 노란색으로 표시된 신규 매장을 찾아 가입증명서 PDF를 자동으로 만들어드려요. 📄"
    )
...
# ===================== 가입증명서 자동 생성 =====================
# 정산양식 엑셀에서 노란색 음영으로 표시된 신규 매장을 찾아, 예시 PDF와 같은 양식의
# 가입증명서를 자동으로 만들어주는 기능. 원본 서식(배경/직인 이미지)은 그대로 두고
# 매장별로 달라지는 값(점포명/주소/보험기간/보험료 등)만 흰색으로 덮은 뒤 새로 그려넣는 방식.

CERT_YELLOW_RGB = "FFFFFF00"
...
def _extract_new_stores_from_settlement_xlsx(file_bytes: bytes, filename: str) -> list[dict] | None:
    """정산양식 엑셀 첫 시트에서 노란색 음영 신규 매장 행을 찾아 가입증명서용 데이터로 변환.
    이 형식이 아닌 것 같으면 None 반환."""
    try:
    ...
    policy_no = m.group(1) if m else ""

    results = []
    for row in ws.iter_rows(min_row=4, max_row=ws.max_row):
        name_cell = row[4] if len(row) > 4 else None
        if name_cell is None or not name_cell.value:
            continue
        try:
            rgb = name_cell.fill.fgColor.rgb if name_cell.fill and name_cell.fill.fgColor else None
        except Exception:
            rgb = None
        if rgb != CERT_YELLOW_RGB:
            continue

        def val(idx):
            return row[idx].value if len(row) > idx else None

        store_name = str(val(4)).strip()
        store_code = str(val(5) or "").strip()
        address = str(val(9) or "").strip()
        address = re.sub(r"(?<=\S)\(", " (", address)
        stock_amt = int(val(10) or 0)
        facility_amt = int(val(11) or 0)
        start_date = val(17)
        end_date = val(18)
        premium = val(19)
        ...
        results.append({
            ...
            "start_date": start_date.strftime("%Y.%m.%d") if hasattr(start_date, "strftime") else str(start_date or ""),
            "start_date_yymmdd": start_date.strftime("%y%m%d") if hasattr(start_date, "strftime") else "",
            "end_date": end_date.strftime("%Y.%m.%d") if hasattr(end_date, "strftime") else str(end_date or ""),
            "store_name": store_name,
            "address": address,
            "stock_amt": stock_amt,
            "facility_amt": facility_amt,
            "premium": round(float(premium)) if premium else 0,
        })
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    stores = _extract_new_stores_from_settlement_xlsx(file_bytes, filename)
    if stores is None:
        await update.message.reply_text(
            "이 엑셀 형식은 알아보지 못했어요. 첫 시트에 노란색 음영으로 표시된 신규 매장이 있는 정산양식인지 확인해주세요."
        )
        return

    if not stores:
        await update.message.reply_text("노란색 음영으로 표시된 신규 매장을 찾지 못했어요.")
        return

    await update.message.reply_text(f"신규 매장 {len(stores)}곳을 찾았어요. 가입증명서를 만들고 있어요...")
    ...
    await update.message.reply_text("✅ 모든 신규 매장 가입증명서를 보내드렸어요.")
