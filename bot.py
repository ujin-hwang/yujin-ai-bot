...
async def _resolve_place(text: str, places: dict) -> dict | None:
    ...
    try:
        return await _kakao_geocode_keyword(t)
    except Exception:
...
async def handle_location(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # 길찾기 진행 중이면 그쪽으로 처리
    route_query = context.user_data.get("route_query")
    if route_query:
        state = route_query["state"]

        if state == "await_current_location":
            # "내 현재위치에서 ~까지" 같은 요청 중 현재 위치를 기다리던 상황
            ...
            if dest_text:
                ...
                await update.message.reply_text(f"현재 위치: {address}\n\n도착지는 어디인가요?")
            return

        if state == "from":
            route_query["from"] = place
            route_query["state"] = "to"
            await update.message.reply_text(f"출발지: {address}\n\n도착지는 어디인가요?")
        else:
            route_query["to"] = place
            context.user_data.pop("route_query", None)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if route_query:
        places = _load_places()
        place = await _resolve_place(user_text, places)
        if not place:
            await update.message.reply_text("위치를 찾지 못했어요. 다른 이름으로 다시 입력하거나 위치를 공유해주세요.")
            return
        if route_query["state"] == "from":
            route_query["from"] = place
            route_query["state"] = "to"
            label = place.get("name") or place.get("address")
